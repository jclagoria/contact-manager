...
export class ContactService {
    ...
    constructor(authHttp: AuthHttp) {
        ...
        this.contacts = new Observable(observer => this._contactObserver = observer).share;
        this.contact = new Observable(observer => this._contactObserver = observer).share();
    }

    public getAll(){
        return this._authHttp.get(`${this._url}`, { headers: contentHeaders} ).map((res: Response) => res.json())
                .map(data => {
                    ...
                    this._contactsObserver.next(this._dataStore.contact);
            }, err => console.error(err));
    }

    public update(contact: Contact) {
        return this._authHttp.put(`${this._url}/${contact._id}`, this._serialize(contact),
